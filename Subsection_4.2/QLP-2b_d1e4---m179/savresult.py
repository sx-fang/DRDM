...
def save_hist(hist_dict, sav_prefix):
    ...
    hist_df.to_csv(f'{sav_prefix}log.csv', index=False)
    it_arr = np.array(hist_df['it'])

    plot_hist(sav_prefix + 'lossmart_hist.pdf', (it_arr, ),
              (hist_df['mart_loss'], ))

    g1_hist = hist_df['g1_loss'].to_numpy()
    ylim0, ylim1 = ylim_for_g1loss(g1_hist)
    plot_hist(sav_prefix + 'g1_loss.pdf', (it_arr, ), (g1_hist, ),
              yscal='linear',
              ylim0=ylim0,
              ylim1=ylim1)

    if ("cost" in hist_dict) and ("vtrue_on_x0cost" in hist_dict):
...
def plot_hist_summary(hist_df: pd.DataFrame, sav_name: str):
    ...
    plot_hist(sav_name + 'summary_maloss.pdf', (it_arr, ),
              (hist_df['mart_loss', 'mean'], ),
              std_arr_list=(hist_df['mart_loss', 'std'], ),
              lable_list=None,
              yscal='log',
              alpha=1.0,
              two_side_std=False)

    mean_g1loss = hist_df['g1_loss', 'mean'].to_numpy()
    ylim0, ylim1 = ylim_for_g1loss(mean_g1loss)
    plot_hist(sav_name + 'summary_g1loss.pdf', (it_arr, ), (mean_g1loss, ),
              std_arr_list=(hist_df['g1_loss', 'std'], ),
              lable_list=None,
              yscal='linear',
              alpha=1.0,
              two_side_std=False,
