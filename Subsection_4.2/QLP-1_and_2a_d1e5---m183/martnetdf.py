...
def make_rate_limiter(print_gap=1.0) -> Callable[[], bool]:
    ...
    This helps control how often certain operations (like logging) happen. If distributed training is in use, it synchronizes the print decision and timestamp across all processes by broadcasting a flag and the updated timestamp.
    """
    last_time = None
    use_dist = isin_ddp()
...
class PathSampler(object):
    ...
    def gen_xtpath(self, num_path, ctr_func=None):
        ...
        if ctr_func is None:
            xt = self.pde.gen_pilpath(x0, self.num_dt)
        else:
            xt, _ = self.pde.gen_syspath(x0, self.num_dt, ctr_func)
...
class MartNet(metaclass=abc.ABCMeta):
    ...
    def __init__(self,
        ...
        self.rank = rank
        self.device = next(nets[0].parameters()).device
...
def train_martnet(
    ...
    scaler = GradScaler(device=martnet.device, enabled=enable_scaler)
