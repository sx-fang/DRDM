...
class PDE(metaclass=abc.ABCMeta):
    ...
    def plot_xtpil(self,
        ...
        xt_pil = self.gen_pilpath(x0, num_dt)
        t = torch.linspace(self.t0, self.te, xt_pil.shape[0])

        plt.figure(figsize=(8, 3))
        y_values = xt_pil[..., dim_of_x].cpu().detach().numpy()
        t_values = t.cpu().detach().numpy()
...
class PDEwithVtrue(PDE):
    ...
    def res_on_path(
        ...
        with torch.no_grad():
            if ctrfunc_syspath is None:
                xt = self.gen_pilpath(x0, num_dt)
            else:
                xt, _ = self.gen_syspath(x0, num_dt, ctrfunc_syspath)
    ...
    def plot_res1d(self,
        ...
        if ctrfunc_syspath is None:
            xt_path = self.gen_pilpath(x0_path, numdt_scatter_path)
        else:
        ...
        t = t.unsqueeze(-1).unsqueeze(-1)
        self.plot_vtx_1d(t, xt_path, sav_prefix)
...
# class BVP(PDEwithVtrue):
#     """
#     Boundary Value Problem (BVP) where v(t, x) is known.
...
#         ''' Additional loss typically used for boundary condition.
#         '''
