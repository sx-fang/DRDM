...
from martnetdf import DNNtx, EigenFuncValue, PathSampler, train_martnet
...
def solve(config, use_dist, rank, sav_name=None):
    ...
    if sav_name is not None:
        ...
        if isinstance(example, HJB):
            ctrfunc_syspath = martnet.unn
        else:
            ctrfunc_syspath = martnet.vnn
...
def task_on_cpu(config, sav_name: str):
    ...
    for r in range(rep_time):
        if issav_everytime is True:
            ...
            savname_r = None
        hist_dict, rdict_list = solve(config, False, None, sav_name=savname_r)
        hdict_list.append(hist_dict)
    ...
    if sav_name is not None:
        ...
        path_summ.to_csv(f'{sav_path}summary_path.csv')
        plot_hist_summary(hist_summ, sav_path)
        plot_path_summary(hist_summ, sav_path)
    return hist_summ
